package de.dwslab.riskmanagement.abduction.model;

import java.util.ArrayList;
import java.util.HashSet;

import de.dwslab.riskmanagement.abduction.model.expressions.EqualVariableExpression;
...
import de.dwslab.riskmanagement.abduction.model.expressions.ThresholdExpression;
...
public class Model {
    ...
    private final ArrayList<FormulaAbstract> formulars = new ArrayList<FormulaAbstract>();
    private final HashSet<Type> types = new HashSet<Type>();
    private final HashSet<PredicateAbstract> observedPredicates = new HashSet<PredicateAbstract>();
    private final HashSet<PredicateAbstract> hiddenPredicates = new HashSet<PredicateAbstract>();
    ...
    public void addFormulas(FormulaAbstract... formulars) {
        for (FormulaAbstract formular : formulars) {
            this.addFormula(formular);
        }
    }
        ...
        for (FormulaAbstract f : this.getFormulas()) {
            ...
            for (IfExpression e : f.getIfExpressions()) {
                if (e instanceof ThresholdExpression) {
                    System.err.println("Threshold Expressions are not supported in TheBeast " + e
                            + ". Rewrite formula f " + f);
                    ...
                    System.err.println("Equal Variable Expressions are not supported in TheBeast "
                            + e + ". Rewrite formula f " + f);
                }
            }
