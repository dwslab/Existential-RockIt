package de.dwslab.riskmanagement.abduction.model.predicates;
...
public class PredicateDouble extends PredicateAbstract {
    ...
    /**
     * In the groundvalues, the last position of the Array contains the double value (encoded as
     * string).
     *
     */
    public void setGroundAndDoubleValues(ArrayList<String[]> groundValues) throws ParseException {
        ...
        if (this.getTypes().size() > 0 && groundValues.get(0).length != this.getTypes().size() + 1) {
            ...
            throw new ParseException(
                    "PredicateDouble: "
                            + this.getName()
                            + " - The size of the line input has to be equal to the size of the types attribute.");
        }
        ...
        for (String[] sArray : groundValues) {
            ...
            try {
                ...
                throw new ParseException("In PrdicateDouble " + this.getName()
                        + " is a Double value which can not be parsed to a double number: "
                        + sArray[lastPos]);
            }
    ...
    public void addGroundValueLine(double value, String... values) throws ParseException {
        /*
         * if(value <0){
         * throw new ParseException("Predicate: " + this.getName() +
        ...
        if (values.length != getTypes().size()) {
            throw new ParseException(
                    "Predicate: "
                            + this.getName()
                            + " - The size of the line input has to be equal the size of the types attribute. Add types first."
                            +
                            " actual size line: " + values.length + " actual size types "
                            + getTypes().size());
        }
    ...
    public PredicateDouble(String name, boolean hidden, ArrayList<Type> types,
            ArrayList<String[]> groundValues) throws ParseException {
        this.setName(name);
        this.setHidden(hidden);
        if (groundValues == null) {
            throw new ParseException("Predicate: " + this.getName()
                    + " - Ground value axiom must not be zero.");
        }
        if (types == null) {
            throw new ParseException("Predicate: " + this.getName()
                    + " - Types axiom must not be zero.");
        }
    ...
    public String toLongString() {
        ...
        if (this.getGroundValues().size() > 0) {
            for (int i = 0; i < this.getGroundValues().size(); i++) {
                ...
                for (String element : groundValue) {
                    sb.append("\"").append(u.getConstant(element)).append("\"").append(", ");
                }
    ...
    public String output() {
        ...
        sb.append("PredicateDouble " + this.getName() + "P = new PredicateDouble(\""
                + this.getName() + "\"," + this.isHidden() + ",");
