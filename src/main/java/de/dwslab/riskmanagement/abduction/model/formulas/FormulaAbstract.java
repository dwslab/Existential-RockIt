...
public abstract class FormulaAbstract implements Comparable<FormulaAbstract> {
    ...
    public abstract HashSet<de.dwslab.riskmanagement.abduction.model.predicates.PredicateAbstract> getAllHiddenPredicatesSet();
    ...
    public void setForVariables(VariableType... forVariables) {
        for (VariableType forVariable : forVariables) {
            this.forVariables.add(forVariable);
        }
    }
    ...
    private void checkIfExpression(IfExpression ex) throws ParseException {
        if (ex.getClass().equals(PredicateExpression.class)) {
            ...
            if (exPred.getPredicate().isHidden()) {
                throw new ParseException("Error in the if part of the formular. The predicate "
                        + exPred.getPredicate().getName()
                        + " is set to hidden. But only observed predicates are allowed here.");
            }
        }
    }
