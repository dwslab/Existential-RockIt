...
public abstract class PredicateAbstract implements Comparable<PredicateAbstract> {
    ...
    public PredicateAbstract(String name, boolean hidden, ArrayList<Type> types,
            ArrayList<ArrayList<String>> groundValues) throws ParseException {
        this.name = name;
        this.hidden = hidden;
        if (groundValues == null) {
            throw new ParseException("Predicate: " + this.name
    ...
    public void addType(Type type) throws ParseException {
        if (this.groundValues.size() > 0 && this.groundValues.get(0).length > 0
                && types.size() >= this.groundValues.size()) {
            throw new ParseException("Predicate: " + this.name
                    + " - The size of the types must not extend the size of the line input.");
        }
        this.types.add(type);
    }
