package de.dwslab.riskmanagement.abduction.model.types;
...
public class Type implements Comparable<Type> {
     ...
     *
     * If the predicate has more than one "row" in ground values (e.g. more than one Type
     * assignment),
    ...
    public void setGroundValuesPredicate(Predicate predicate) throws ParseException {
        if (predicate == null) {
            throw new ParseException(
                    "Predicates must not equal null when added to type. You tried to add a null predicate to type "
                            + this.getName());
        }
        if (predicate.getTypes().size() > 1) {
            throw new ParseException(
                    "Predicates which are used to set ground values must not have more than one type. This happens when trying to assign predicate "
                            + predicate.getName() + " to type " + this.getName());
        }
    ...
    public String output() {
        ...
        if (this.groundValuesPredicate != null) {
            result = result + this.getName() + ".values = "
                    + this.getGroundValuesPredicate().getName();
        }
        return result;
     ...
     * if(this.groundValuesPredicate==null){
     * throw new
     * ParseException("Can not determine the size if no ground values predicate is set. Type name: "
     * + this.name);
     * }
