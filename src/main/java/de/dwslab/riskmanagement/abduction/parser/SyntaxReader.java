...
import de.dwslab.riskmanagement.abduction.model.formulas.FormulaAbstract;
...
import de.dwslab.riskmanagement.abduction.model.formulas.FormulaObjective;
...
public class SyntaxReader {
    ...
    IOException {
        ...
        ArrayList<PredicateAbstract> remainingGroundValues = this.integrateGroundValuesInModel(
                groundValues, model);

        // integrate the hidden predicate values. More precisely, transform them into observed
        // predicate values and add a hard formular to connect them to the hidden pred.
        ...
        // containing these ground values.
        this.assignTypePredicates(model);

        // figures out which variables are not yet bound in formulas and add the "type predicate" of
        // this variable to the if restriction.
        this.addUnboundVariablesPredicateTypes(model);

        // processes string variables - transform them to extra observed predicates and add them to
        // types.
    ...
    public Model getGroundValuesForLearning(String groundValueFile, Model model)
    ...
    public Model getModelForEvaluation(String modelFile, String groundValueFile, String resultFile)
    ...
    private ArrayList<PredicateAbstract> integrateResultsInModel(
            Collection<PredicateAbstract> results, Model model) throws ParseException {
    ...
    /**
     * If only one positive hidden predicate exist in soft formula and if the double variable is not
     * null, it is transformed to an objective formula.
     *
     * @return
    ...
    private void transformSoftToObjectiveFormulas(Model model) throws ParseException {
        ...
        for (FormulaAbstract f : model.getFormulas()) {
            if (f instanceof FormulaSoft) {
                ...
                if (fs.getRestrictions().size() == 1 && fs.getRestrictions().get(0).isPositive()
                        && fs.getDoubleVariable() != null) {
    ...
    /**
     * Figures out the ground values for types and assigns a new predicate to each type containing
     * these ground values.
     *
     * @param groundValues
     * @param model
     ...
     * @throws ParseException
     */
    private ArrayList<PredicateAbstract> integrateGroundValuesInModel(
            TreeSet<PredicateAbstract> groundValues, Model model) throws ParseException {
        ...
        for (PredicateAbstract groundValue : groundValues) {
            if (observedPredicates.contains(groundValue)) {
                ...
                if (observedPredicate instanceof PredicateDouble
                        && groundValue instanceof PredicateDouble) {
    ...
    private Model integrateHiddenPredicateValuesWithNewFormula(Model model,
            ArrayList<PredicateAbstract> remainingGroundValues) throws ParseException {
        ...
        // introduce a hard formula and another predicate for those who are part of the hidden
        // predicates
        ...
        for (PredicateAbstract hiddenPred : hiddenPreds) {
            ...
            if (index >= 0) {
                ...
                if (hiddenPred instanceof PredicateDouble) {
                    ...
                    formularSoft.addIfExpression(new PredicateExpression(true, newObservedPred,
                            varList));
                    name.append("F");
                    formularSoft.setName(name.toString());
                    formularSoft.setForVariables(forVariables);
                    formularSoft.setDoubleVariable(ddd);
                    formularSoft.setRestrictions(new PredicateExpression(positive, hiddenPred,
                            varList));
                    ...
                    formularH.addIfExpression(new PredicateExpression(true, newObservedPred,
                            varList));
                    name.append("F");
    ...
    /**
     * Figures out the groundings of all types and assignes ground values to the types. This is done
     * by creation of a dummy predicate
     * per type.
     *
     * @param model
     * @throws ParseException
     ...
     * types.
     *
     * If predicates are hidden and if variables are StringVariables, they are transformed as
     * followed:
     *
     * Example:
     * Original:
                    ...
                    for (int i = 0; i < vars.size(); i++) {
                        ...
                        if (var instanceof VariableString) {
                            ...
                            if (/* Parameters.CONVERT_STRING_VALUES && */expr.isPositive()) {
                                String name = new StringBuilder().append("varString")
                                        .append(processStringVariableCounter).toString();
    ...
    private void addUnboundVariablesPredicateTypes(Model model) throws ParseException {
        for (FormulaAbstract formularAbstract : model.getFormulas()) {
            if (formularAbstract instanceof FormulaHard || formularAbstract instanceof FormulaSoft
                    || formularAbstract instanceof FormulaCardinality) {
                ...
                for (VariableAbstract varAbstr : unboundVariableTypes) {
                    if (varAbstr instanceof VariableType) {
                        ...
                        if (t.getGroundValuesPredicate() == null) {
                            Predicate typePredicate = new Predicate(t.getName() + "_typePred",
                                    false, t);
                            t.setGroundValuesPredicate(typePredicate);
                        }
                        Predicate groundValuesP = t.getGroundValuesPredicate();
                        formular.addIfExpression(new PredicateExpression(true, groundValuesP,
                                varAbstr));
                ...
                if (formular.isConjunction()) {
                    ...
                    for (PredicateExpression expr : formular.getRestrictions()) {
                        FormulaHard newHard = new FormulaHard(
                                formular.getName() + i, formular.getForVariables(),
                                formular.getIfExpressions(), new ArrayList<PredicateExpression>(),
                                false);
                ...
                if (fs.getRestrictions().size() > 1) {
                    for (PredicateExpression expr : fs.getRestrictions()) {
                        ...
                        FormulaSoft newSoft = new FormulaSoft(
                                fs.getName() + "simplified" + i, fs.getForVariables(),
                                fs.getIfExpressions(), fs.getDoubleVariable(),
                                new ArrayList<PredicateExpression>(), false);
